...
- Counter-scale the weight-control overlay with the same easing curve as its eigenface tile so the control remains visually stationary while the eigenface enlarges. Hover and focus-within must use the same tile scale, so engaging a slider during hover never triggers a second enlargement. Keep the overlay on its own compositor layer to prevent a late rasterization snap as the dissolve completes.
