...
- Counter-scale the weight-control overlay with the same easing curve as its eigenface tile so the control remains visually stationary while the eigenface enlarges. Keep the overlay on its own compositor layer to prevent a late rasterization snap as the dissolve completes.
