...
- Keep the reconstruction topline limited to its section label. Below the image, show matching two-line metric blocks on one row: `Variance retained` above its percentage first, then `Dimensions` above its interactive numeric value. Remove the redundant `Projected from 5,000 faces` footer metric. The dimensions slider discloses into the free space to the right of the numeric value so retained variance never shifts when the control opens. On pointer devices, it disappears whenever Dimensions is no longer hovered; keyboard focus and touch disclosure remain available.
...
- Keep the structural hairline static while tiles scale. Render interaction emphasis as a compositor-backed inset shadow rather than an animated border, and do not transition the perimeter itself. Exactly the hovered tile receives the stable inset; only its scale and control disclosure animate, preventing subpixel flashes and overlapping boundary fades.
...
- Store the baseline reconstruction and eight raw eigenvectors as little-endian Float32 values.
...
- Under `prefers-reduced-motion: reduce`, remove tile scaling and reduce disclosure transitions to effectively instantaneous state changes.
